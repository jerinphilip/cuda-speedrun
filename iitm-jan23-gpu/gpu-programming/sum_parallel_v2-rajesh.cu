...
__global__ void dk(int *a, int n, int iteration) {
  ...
  if (id < n) {
    ...
    // if( n % 2 == 1 && id == n-1 ){
    //	a[index] = a[index] + a[ index + shift_index ] +a[index + 2 *
    //shift_index ]; printf("a[%d] = a[%d] + a[%d] + a[%d] = %d \n",index,index,
    // index + shift_index ,index + 2 * shift_index , a[index]);
    //}
    // else{
    a[index] = a[index] + a[index + shift_index];
    // printf("a[%d] = a[%d] + a[%d] = %d\n",index, index, index + shift_index ,
    // a[index]);
    //}
  }
  //__syncthreads();
