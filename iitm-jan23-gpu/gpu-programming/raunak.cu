...
__global__ void stencil(int *d_input, int M, int N) {
  ...
  if (thidx > 0 && thidy > 0 && thidx < N - 1 && thidy < M - 1)
    write_this =
        0.2 * (d_input[thidy * N + thidx] + d_input[(thidy + 1) * N + thidx] +
               d_input[(thidy - 1) * N + thidx] +
               d_input[thidy * N + thidx + 1] + d_input[thidy * N + thidx - 1]);
  __syncthreads();
  if (threadIdx.x == 0 && threadIdx.y == 0) atomicAdd((int *)&counter, 1);
  while (counter < tot_blocks)
    ;  // Waits here for infinite time.
  if (thidx > 0 && thidy > 0 && thidx < N - 1 && thidy < M - 1)
