...
__global__ void k1(unsigned *nelements) {
  ...
  for (unsigned ii = 0; ii < nelements[id]; ++ii) worklist[index + ii] = id;
}
__global__ void k2() {
  printf("Number of threads = %d, worklist size = %d\n", N, wlsize);

  for (unsigned ii = 0; ii < wlsize; ++ii) printf("%d ", worklist[ii]);
  printf("\n");
}
int main() {
  ...
  cudaMemcpy(nelements, hnelements, N * sizeof(unsigned),
