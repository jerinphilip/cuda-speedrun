...
__global__ void transformKernelGlobalMemory(float *outputData, int width,
  ...
  // read from texture and write to global memory
  outputData[y * width + x] = texglobal[y * width + x];
}
...
void runTest(int argc, char **argv) {
  ...
  float *texglobal;
  cudaMalloc(&texglobal, size);
  cudaMemcpy(texglobal, hData, size, cudaMemcpyHostToDevice);
  dim3 dimBlock(8, 8, 1);
  ...
  for (unsigned ii = 0; ii < 100; ++ii) {
    transformKernel<<<dimGrid, dimBlock, 0>>>(dData, width, height, angle);
    // transformKernelGlobalMemory<<<dimGrid, dimBlock, 0>>>(dData, width,
    // height, angle, texglobal);
