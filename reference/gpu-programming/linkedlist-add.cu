...
__global__ void listAdd() {
  ...
  do {
    ...
    actualoldhead = (struct node *)atomicCAS((unsigned long long *)&head,
                                             (unsigned long long)myoldhead,
                                             (unsigned long long)newnode);
  } while (actualoldhead != myoldhead);
}
__device__ void listPrint(struct node *ptr) { printf("%d ", ptr->data); }
__global__ void listPrint() {
  int nnodes = 0;
  for (struct node *ptr = head; ptr; ptr = ptr->next, ++nnodes) listPrint(ptr);
  printf("\nNumber of nodes = %d\n", nnodes);
}
int main() {
